...
source("../01_identification_of_mutation_subtypes/helper_functions/convertgmt_mod.R")
...
donor_mut <- donor[donor$IGHV=="MUT",]
x_mut <- x[row.names(x) %in% donor_mut$submitted_donor_id,]
...
t_path_mut_score <- t_path_mut_score[-c(35,40,48,283),]
...
donor_cp$cluster_k3 <- k_modes(cc_cp_1_combined$`3`)
...
relapse$cluster_k3 <- donor_cp$cluster_k3
...
table(relapse$cluster_k3)
relapse_fit_k3 <- survfit(relapse_obj ~ cluster_k3, data=relapse)
g2 <- ggsurvplot(relapse_fit_k3, data = relapse, pval = TRUE,
                 xlab = "Time to treatment (years)", ylab = "Probability",
